...
BYTE benchopcode[BENCHOPCODES] = {0x06,0x16,0x24,0x45,0x48,0x65,0x68,0x76,
...
cpuexecutetype cpuexecutefunc[3] = {NULL,NULL,NULL};
cpugetcodetype cpugetcodefunc[3] = {NULL,NULL,NULL};
cpuinittype    cpuinitfunc[3]    = {NULL,NULL,NULL};
HINSTANCE      cpulibrary[3]     = {(HINSTANCE)0,(HINSTANCE)0,(HINSTANCE)0};
  ...
  // OF THE CYCLE COUNT USING ONLY 16 BITS.
    ...
    if (cycles <= 0xFFFF)
      ...
      do {
        ...
        if (cpuexecutefunc[cpuemtype])
          result += cpuexecutefunc[cpuemtype](cyclestoexec);
        else
